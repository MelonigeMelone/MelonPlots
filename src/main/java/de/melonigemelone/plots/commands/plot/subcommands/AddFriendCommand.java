...
public class AddFriendCommand {
    ...
    public static boolean handleCommand(Player player, String label, String[] args) {
        if(args.length != 2) {
            player.sendMessage(GeneralMessages.COMMAND_USAGE.getMessage(true).replaceAll("%command%","/gs addFriend (Name)"));
            return true;
        }
        ...
        if(plot.isFriend(targetUUID)) {
            player.sendMessage(GeneralMessages.PREFIX.getMessage(false) + "§cDer Spieler ist bereits auf deinem Grunstück als Freund hinzugefügt!");
            return true;
        }

        plot.addFriend(targetUUID);
        player.sendMessage(GeneralMessages.PREFIX.getMessage(false) + "§aDu hast den Spieler erfolgreich als Freund hinzugefügt!");
        return true;
    }
}
